...
## 4.9 texture 轴：per-patch charting 的必要性消融（2026-07-13）

texture 的外部对比（把 baseline mesh 强加一个 charting）与 edit 几何-GT 同样有争议，故改做
**可辩护的消融**：per-patch charting（ours）vs single-chart（整物体一张切平面）——直接回答
§4.1 提出的「charting 到底重不重要」。为堵「分辨率不公平」，single-chart 另给一个**总纹素
匹配**（res≈32·√#patches）的档。

| 场景 | #patches | per-patch PSNR | single-chart@32 | single-chart@纹素匹配 |
|---|---:|---:|---:|---:|
| scan24 | 208 | 30.11 dB | 11.40 | 18.28（@462）|
| scan65 | 215 | 35.34 dB | 11.58 | 19.66（@469）|
| scan105 | 243 | 33.70 dB | 15.99 | 24.58（@499）|

**结论**：即便给 single-chart **同样的总纹素预算**，per-patch 仍领先 **6–15 dB**。因为单张
切平面投影无法表示曲面/闭合面（对侧的点塌到同一 UV），是**拓扑限制而非分辨率**。这**可辩护
地证明 per-patch charting 对纹理保真是必要的**，不是风格选择。与 §4.1/4.3 一致且互补：
...
## 4.11 asset 可用性演示：语义部件编辑（CPU，2026-07-13）

第二个可用性演示（能被美术编辑）：在 scan105 上选一个语义部件（密集区一簇 30 个认证 patch，
2287 顶点，约占 mesh 10%），刚性旋转 35°，两种绑定传播对比。脚本
`scripts/demo_edit_propagation.py`，图 `figures/edit_demo/edit_before_after.png`。

| 绑定 | 泄漏点比例 | 泄漏顶点 | 边界位移均值 |
|---|---:|---:|---:|
| **certified patch binding** | **0.00%** | 0 | 0 |
| proximity binding（6% bbox 选区）| 19.72% | 3847 | 0.83% bbox |

**结论**：certified binding 精确移动那 2287 个顶点、其余**零位移**；而 mesh-free 的 proximity
选区为圈住该部件，**误拖 3847 个相邻顶点**（比部件本身还多，占 mesh 近 20%）。图中：左panel
只有蓝色目标动，右panel 一大团红色"本应固定却被拖走"。这与 §4.8 呼应——watertight baseline
无 patch 身份，编辑**只能**用 proximity，就会产生这种附带破坏；认证 patch 提供语义 clean cut。
这是「零泄漏 edit」从 benchmark 数字（§E1 leak reduction）落到**可视的编辑行为**上的实用价值。

至此 asset 可用性有两个 CPU 端到端演示：**物理碰撞正确（§4.10）+ 编辑干净不破坏邻域
（§4.11）**。剩下「引擎导入/渲染」（Blender A1）需带 GUI/GPU 的机器，属用户侧。
